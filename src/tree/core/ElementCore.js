...
import FlexTarget from '../../flex/FlexTarget.js'

import ElementTexturizer from './ElementTexturizer.js'
...
export default class ElementCore {
  ...
  setParent (parent) {
    if (parent !== this._parent) {
      ...
      if (!this._shader) {
        const newShaderOwner = parent && !parent._renderToTextureEnabled ? parent._shaderOwner : null
  ...
  disableZContext () {
    ...
    if (this._zContextUsage > 0) {
      const newZParent = this._parent.findZContext()
  ...
  set shader (v) {
    ...
    if (!v && prevShader) {
      // Disabled shader.
      const newShaderOwner = this._parent && !this._parent._renderToTextureEnabled ? this._parent._shaderOwner : null
      this._setShaderOwnerRecursive(newShaderOwner)
    } else if (v) {
      // Enabled shader.
  ...
  updateRenderToTextureEnabled () {
    ...
    const v = this.texturizer._enabled
          ...
          if (!this._renderToTextureEnabled || !this._texturizer || !this._texturizer.renderOffscreen) {
            ...
            if (!withinMargin) {
              ...
              if (this._recBoundsMargin) {
                withinMargin = !(
                  ex < this._scissor[0] - this._recBoundsMargin[2] ||
