...
import ElementCore from './core/ElementCore.js';
...
import ElementChildList from './ElementChildList.js';

export default class Element {
  /**
   *
   * @param {import('./Stage').default} stage
   */
  constructor(stage) {
    this.stage = stage;
  ...
  getAncestorAtDepth(depth) {
    const levels = this.getDepth() - depth;
    if (levels < 0) {
      return null;
    }
  ...
  getSharedAncestor(c) {
    let o1 = this;
    let o2 = c;
    const l1 = o1.getDepth();
    const l2 = o2.getDepth();
    if (l1 > l2) {
  ...
  _isEnabled() {
    return (
      this.__core.visible &&
      this.__core.alpha > 0 &&
      (this.__parent ? this.__parent.__enabled : this.stage.root === this)
    );
  }
  ...
  _unsetTagsParent() {
    if (this.__tags) {
      this.__tags.forEach((tag) => {
        ...
        let p = this;
        while ((p = p.__parent)) {
          const parentTreeTags = p.__treeTags.get(tag);
    ...
    if (this.__treeTags) {
      if (!this.__tagRoot) {
        ...
        if (n > 0) {
          for (let i = 0; i < n; i++) {
            const tagSet = this.__treeTags.get(tags[i]);

            // Remove from treeTags.
            let p = this;
            while ((p = p.__parent)) {
              const parentTreeTags = p.__treeTags.get(tags[i]);
  ...
  _getByTag(tag) {
    ...
    const t = this.__treeTags.get(tag);
  ...
  removeTag(tag) {
    const i = this.__tags.indexOf(tag);
    if (i !== -1) {
      ...
      if (p) {
        do {
          const list = p.__treeTags.get(tag);
          if (list) {
            list.delete(this);
          }
        } while (!p.__tagRoot && (p = p.__parent));
  ...
  fastForward(property) {
    if (this._transitions) {
      const t = this._transitions[property];
      if (t && t.isTransition) {
        t.finish();
      }
    }
  ...
  _setTransition(property, settings) {
    if (!settings) {
      ...
      const current = this._transitions[property];
  ...
  getSmooth(property, v) {
    const t = this._getTransition(property);
    if (t && t.isAttached()) {
