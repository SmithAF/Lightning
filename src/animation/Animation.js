...
export default class Animation extends EventEmitter {
  ...
  play () {
    if (this._state === Animation.STATES.PAUSED) {
      ...
      this.settings.stopMethod == AnimationSettings.STOP_METHODS.REVERSE
    ) {
      // Continue.;
      this._state = Animation.STATES.PLAYING
  ...
  stop () {
    ...
    if (
      (this.settings.stopMethod === AnimationSettings.STOP_METHODS.IMMEDIATE && !this._stopDelayLeft) ||
      this._delayLeft > 0
    ) {
  ...
  isActive () {
    return (
      (this._state == Animation.STATES.PLAYING || this._state == Animation.STATES.STOPPING) &&
      this._element &&
      this._element.attached
    )
  }
  ...
  apply () {
    if (this._state === Animation.STATES.STOPPED) {
      ...
      if (
        this._state === Animation.STATES.STOPPING &&
        this.settings.stopMethod === AnimationSettings.STOP_METHODS.FADE
      ) {
