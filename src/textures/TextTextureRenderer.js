  ...
  _calculateRenderInfo() {
    ...
    if (this._settings.maxLines && lines.length > this._settings.maxLines) {
      ...
      if (this._settings.maxLinesSuffix) {
        ...
        const w = this._settings.maxLinesSuffix ? this.measureText(this._settings.maxLinesSuffix) : 0;
        const al = this.wrapText(usedLines[usedLines.length - 1], wordWrapWidth - w, letterSpacing);
    ...
    for (let i = 0; i < lines.length; i++) {
      const lineWidth = this.measureText(lines[i], letterSpacing);
