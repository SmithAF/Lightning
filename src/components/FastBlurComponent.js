...
import DefaultShader from '../renderer/webgl/shaders/DefaultShader.js'
import C2dBlurShader from '../renderer/c2d/shaders/BlurShader.js'
import Shader from '../tree/Shader.js'
import MultiSpline from '../tools/MultiSpline.js'
...
class WebGLFastBlurComponent extends Component {
    ...
    return {
      ...
      Layers: {
        L0: {
          ...
          visible: false,
          Content: { shader: { type: BoxBlurShader } }
        },
        L1: {
          rtt: true,
          onUpdate: onUpdate,
  ...
  _buildLayers () {
    const filterShaderSettings = [
      { x: 1, y: 0, kernelRadius: 1 },
      { x: 0, y: 1, kernelRadius: 1 },
      { x: 1.5, y: 0, kernelRadius: 1 },
      { x: 0, y: 1.5, kernelRadius: 1 }
    ]
  ...
  _updateBlurSize () {
    ...
    const fw = w + paddingX * 2
    const fh = h + paddingY * 2
    this._textwrap.w = fw
    this._wrapper.x = paddingX
  ...
  _update () {
    const v = Math.min(4, Math.max(0, this._amount))
    if (v === 0) {
...
class FastBlurOutputShader extends DefaultShader {
  ...
  beforeDraw (operation) {
    const glTexture = this._otherTextureSource ? this._otherTextureSource.nativeTexture : null
