...
export default class ListComponent extends Component {
  ...
  setIndex (index, immediate = false, closest = false) {
    ...
    if (closest) {
      ...
      const offset = Utils.getModuloIndex(index, nElements)
      const o = Utils.getModuloIndex(this.index, nElements)
      let diff = offset - o
    ...
    const direction = this._horizontal ^ this._invertDirection ? -1 : 1
  ...
  getAxisPosition () {
    const target = -this._scrollTransition._targetValue

    const direction = this._horizontal ^ this._invertDirection ? -1 : 1
    const value = -direction * this._index * this._itemSize
  ...
  update () {
    ...
    const scrollDelta = this._viewportScrollOffset * viewportSize - this._itemScrollOffset * this._itemSize
    v += scrollDelta
    ...
    for (let index = s; direction == -1 ? index <= e : index >= e; direction == -1 ? index++ : index--) {
      const realIndex = Utils.getModuloIndex(index, nElements)

      const element = this.getElement(realIndex)
      item = element.parent
  ...
  getElement (index) {
    const e = this._wrapper.children[index]
  ...
  get element () {
    const e = this._wrapper.children[this.realIndex]
    return e ? e.children[0] : null
  }
...
class ListItems extends ObjectListWrapper {
  constructor (list) {
    const wrap = (item) => {
      const parent = item.stage.createElement()
      parent.add(item)
