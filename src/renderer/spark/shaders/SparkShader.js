  ...
  draw (operation) {
    ...
    if (length) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (glTexture !== tx) {
          if (glTexture.options && glTexture.options.imageRef) {
            const elementPostion = i > 0 ? i - 1 : i
            const precision = this.ctx.stage.getOption('precision')
            const vc = operation.getElementCore(elementPostion)
            this.ctx.stage.platform.paint(
              gl,
              glTexture.options.imageRef,
              vc._worldContext.px * precision,
              vc._worldContext.py * precision,
              vc._colorUl,
              vc
            )
          } else {
      ...
      if (pos < length) {
        if (glTexture.options && glTexture.options.imageRef) {
          const precision = this.ctx.stage.getOption('precision')
          const vc = operation.getElementCore(pos)
          this.ctx.stage.platform.paint(
            gl,
            glTexture.options.imageRef,
            vc._worldContext.px * precision,
