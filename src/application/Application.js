...
export default class Application extends Component {
  ...
  __updateFocusRec() {
    ...
    if (!prevFocusedComponent) {
      ...
      const m = Math.min(this._focusPath.length, newFocusPath.length);
      let index;
    ...
    do {
      ...
      if (ptr === current) {
        ...
        do {
          if (!ptr) {
            current._throwError(
              "Return value for _getFocused must be an attached descendant component but its '" +
                nextFocus.getLocationString() +
                "'"
            );
          }
          newParts.push(ptr);
